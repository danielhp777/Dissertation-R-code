...
model1 <- rpart(logrna ~ ., data = hiv_train, method = "anova")
...
# define training and testing data as DMatrices
...
eta <- c(0.1, 0.3, 0.5) # three choices of eta
max_depth <- c(3,4,5) # three choices of max_depth
best_parameters <- list() # tracks which eta and max_depth works best
best_nrounds <- 0 # tracks the best value for nround parameter
...
xgb.plot.importance(importance) # not used in dissertation
